...
namespace MCeff
{
  std::vector<float> ptBins = {15, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, 110};
                  ...
                  std::string output)
{
  ...
  TH2F* hempty = new TH2F("hempty", ";p_{T} (GeV/c);#alpha #times #epsilon_{reco} #times #epsilon_{sel}", 10, MCeff::ptBins[0], MCeff::ptBins[MCeff::nPtBins-1], 10, 0, 0.1);
  xjjroot::sethempty(hempty, 0, 0.3);
  TH2F* hempty_incl = new TH2F("hempty_incl", ";;#alpha #times #epsilon_{reco} #times #epsilon_{sel}", 5, 0, 5, 10, 0, 0.1);
  ...
  xjjroot::drawtex(0.92, 0.84, "|y| < 2", 0.042, 32, 62);
  xjjroot::drawCMSleft("#scale[1.25]{#bf{CMS}} #it{Simulation}");
  xjjroot::drawCMSright();

  c->cd(2);
  hempty_incl->Draw();
  greff_incl_a->Draw("same");
  greff_incl_b->Draw("same");
  xjjroot::drawtex(0.45, 0.35, Form("%.1f {}^{+ %.1f}_{-  %.1f} #times 10^{-3}", greff_incl_a->GetEfficiency(2)*1000, greff_incl_a->GetEfficiencyErrorUp(2)*1000, greff_incl_a->GetEfficiencyErrorLow(2)*1000), 0.042, 22, 62, fitX::color_a);
  xjjroot::drawtex(0.75, 0.35, Form("%.1f {}^{+ %.1f}_{-  %.1f} #times 10^{-3}", greff_incl_b->GetEfficiency(4)*1000, greff_incl_b->GetEfficiencyErrorUp(4)*1000, greff_incl_b->GetEfficiencyErrorLow(4)*1000), 0.042, 22, 62, fitX::color_b);
  xjjroot::drawtex(0.23, 0.84, "PYTHIA8 + HYDJET", 0.042, 12, 62);
  xjjroot::drawtex(0.92, 0.84, "|y| < 2", 0.042, 32, 62);
